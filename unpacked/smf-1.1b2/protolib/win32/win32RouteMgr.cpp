...
class Win32RouteMgr : public ProtoRouteMgr
{
    public:
        ...
        {
            char ifName[256];
            bool result = ProtoSocket::GetInterfaceName(ifIndex, ifName, 64);
...
bool Win32RouteMgr::SetForwarding(bool state)
{
    ...
    {
        DMSG(0, "Win32RouteMgr::SetForwarding() SetIpStatistics() error: %s\n", GetErrorString());
        return false;
    }
                             ...
                             int&                metric)
{
    ...
    if (!routeTable.Init())
    {
        DMSG(0, "Win32RouteMgr::GetRoute() error initing route table\n");
        return false;
    }
    if (!GetAllRoutes(dst.GetType(), routeTable))
    {
        DMSG(0, "Win32RouteMgr::GetRoute() error getting system route table\n");
        return false;
    }
                             ...
                             int                 metric)
{
    ...
    {
        DMSG(0, "Win32RouteMgr::SetRoute() error: IPv6 not yet supported\n");
        return false;
    }
    if (0 == ifIndex)
    {
        DMSG(0, "Win32RouteMgr::SetRoute() error: ifIndex not specified!\n");
        return false;
    }
	int oldmetric = 0;
	ProtoAddress oldgw;
    ...
    if (NO_ERROR != SetIpForwardEntry(&entry))
    {
        ...
        {
            DMSG(0, "Win32RouteMgr::SetRoute() CreateIpForwardEntry() error\n");
            return false;
        }
        else
        {
                                ...
                                unsigned int        ifIndex)
{
    if (ProtoAddress::IPv4 != dst.GetType())
    {
        DMSG(0, "Win32RouteMgr::DeleteRoute() error: IPv6 not yet supported\n");
        return false;
    }
    if (0 == ifIndex)
    {
        DMSG(0, "Win32RouteMgr::DeleteRoute() error: ifIndex not specified!\n");
        return false;
    }
	MIB_IPFORWARDROW entry;
    memset(&entry, 0, sizeof(MIB_IPFORWARDROW));
    ...
    {
        DMSG(0, "Win32RouteMgr::DeleteRoute() DeleteIpForwardEntry() error\n");
        return false;
    }
    else
    {
                                 ...
                                 ProtoRouteTable&   routeTable)
{
    ...
    if (NULL == buffer)
    {
        DMSG(0, "Win32RouteMgr::GetAllRoutes() new buffer[%lu] error\n", bufferSize);
        return false;
    }
    MIB_IPFORWARDTABLE* fwdTable = (MIB_IPFORWARDTABLE*)buffer;
    if (NO_ERROR != GetIpForwardTable(fwdTable, &bufferSize, FALSE))
    {
        DMSG(0, "Win32RouteMgr::GetAllRoutes() new MIB_IPFORWARDTABLE error\n");
        delete[] buffer;
        return false;
    }
    bool result = true;
    ...
    for (DWORD i = 0 ; i < numEntries; i++)
    {
        ...
        {
            DMSG(0, "Win32RouteMgr::GetAllRoutes() error creating table entry\n");
            result = false;
            break;
        }
    }
