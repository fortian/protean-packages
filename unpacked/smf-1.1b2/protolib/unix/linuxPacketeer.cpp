...
bool LinuxPacketeer::Send(const char* buffer, unsigned int buflen)
{
    ...
    if (result != buflen)
    {
        DMSG(0, "LinuxPacketeer::Send() sendto() error: %s\n", GetErrorString());
        return false;  
    }    
    return true;
