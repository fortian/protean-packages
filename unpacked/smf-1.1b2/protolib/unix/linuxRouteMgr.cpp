...
bool LinuxRouteMgr::SetForwarding(bool state)
{
    ...
    if (fd < 0)
    {
        DMSG(0, "LinuxRouteMgr::SetForwarding() open(/proc/sys) error: %s\n",
                strerror(errno));
        return false;   
    }
    ...
    {
        DMSG(0, "LinuxRouteMgr::SetForwarding() write(/proc/sys) error: %s\n",
            strerror(errno));
        return false;      
    }
...
bool LinuxRouteMgr::Open(const void* /*userData*/)
{
    ...
    {
        DMSG(0, "LinuxRouteMgr::Open() socket(NETLINK_ROUTE) error: %s\n",
                strerror(errno));  
        return false; 
    }
    else
...
bool LinuxRouteMgr::NetlinkCheckResponse(UINT32 seq)
{
    while (1)
    {
        ...
        if (msgLen < 0)
        {
            ...
            {
                DMSG(0, "LinuxRouteMgr::NetlinkCheckResponse() recv() error: %s\n", strerror(errno)); 
                return false;  
            }
        } 
            ...
            if ((msg->nlmsg_pid == (UINT32)pid) && (msg->nlmsg_seq == seq))
            {
                switch(msg->nlmsg_type)
                {
                    case NLMSG_ERROR:
                    {
                        ...
                        if (0 != errorMsg->error)
                        {
                            DMSG(0, "LinuxRouteMgr::NetlinkCheckResponse() recvd NLMSG_ERROR "
                    ...
                    default:
                        DMSG(0, "LinuxRouteMgr::NetlinkCheckResponse() recvd unexpected "
                             ...
                             int                   metric)
{
    ...
    DMSG(6, "LinuxRouteMgr::SetRoute() setting route to %s/%d via ",
    ...
    {    
        DMSG(6, "LinuxRouteMgr::SetRoute() error deleting pre-existing route to %s/%d\n",
                dst.GetHostString(), prefixLen);
    }       
...
#endif // HAVE_IPV6
        default:
            DMSG(0, "LinuxRouteMgr::SetRoute() invalid destination address!\n");
            return false;
            break;
    }
    ...
    if (dst.IsValid())
    {
        ...
        if (prefixLen < addrBits)
        {
            ...
            {
                DMSG(0, "LinuxRouteMgr::SetRoute() invalid address for given mask\n");
                return false;   
            }
        ...
        {
            DMSG(0, "LinuxRouteMgr::SetRoute() invalid mask.\n");
            return false;
        }
        if (!NetlinkAddAttr(&req.msg, sizeof(req), RTA_DST, dst.GetRawHostAddress(), dst.GetLength()))
        {
            DMSG(0, "LinuxRouteMgr::SetRoute() error setting RTA_DST attr.\n");
            return false;
        }
    }
    else
    {
        DMSG(0, "LinuxRouteMgr::SetRoute() error: invalid destination address\n");
        return false;
    }
    ...
    if (gw.IsValid())
    {
        ...
        {
            DMSG(0, "LinuxRouteMgr::SetRoute() error adding RTA_GATEWAY attr.\n");
            return false;
        }
    }
    else if (ifIndex == 0)
    {
        DMSG(0, "LinuxRouteMgr::SetRoute() error: invalid gateway address\n");
        return false;   
    }
    ...
    if (ifIndex != 0)
    {
        ...
        {
            DMSG(0, "LinuxRouteMgr::SetRoute() error adding RTA_OIF attr.\n");
            return false;
        }  
    }
    ...
    if (metric >= 0)
    {
        ...
        {
            DMSG(0, "LinuxRouteMgr::SetRoute() error adding RTA_PRIORITY attr.\n");
            return false;
        }  
    }
    ...
    {
        DMSG(0, "LinuxRouteMgr::SetRoute() send() error: %s\n",
                strerror(errno));
        return false;   
    } 
                                ...
                                unsigned int        ifIndex)
{
    ...
    DMSG(6, "LinuxRouteMgr::DeleteRoute() %u) getting route(s) to dst>%s/%d ",
            debugCount++, dst.GetHostString(), prefixLen);
    int metric;
    while (GetRoute(dst, prefixLen, gw, ifIndex, metric))
    {   
        DMSG(6, "LinuxRouteMgr::DeleteRoute() %u) deleting dst>%s/%d ",
                debugCount++, dst.GetHostString(), prefixLen);
        if (gw.IsValid())
...
#endif // HAVE_IPV6
            default:
                DMSG(0, "LinuxRouteMgr::DeleteRoute() invalid destination address!\n");
                return false;
                break;
        }
        ...
        {       
            ...
            if (prefixLen < addrBits)
            {
                ...
                if (0 != ((0x00ff >> (prefixLen & 0x07)) & bytes[index]))
                {
                    DMSG(0, "LinuxRouteMgr::DeleteRoute() invalid address for given mask\n");
                    return false;   
                }
                while (++index < dst.GetLength())
                {
            ...
            {
                DMSG(0, "LinuxRouteMgr::DeleteRoute() invalid mask.\n");
                return false;
            }
            if (!NetlinkAddAttr(&req.msg, sizeof(req), RTA_DST, dst.GetRawHostAddress(), dst.GetLength()))
            {
                DMSG(0, "LinuxRouteMgr::DeleteRoute() error setting RTA_DST attr.\n");
                return false;
            }
        }
        else
        {
            DMSG(0, "LinuxRouteMgr::DeleteRoute() no valid destination given\n");
            return false;
        }
        ...
        if (gw.IsValid())
        {
            if (!NetlinkAddAttr(&req.msg, sizeof(req), RTA_GATEWAY, gw.GetRawHostAddress(), gw.GetLength()))
            {
                DMSG(0, "LinuxRouteMgr::DeleteRoute() error setting RTA_GATEWAY attr.\n");
                return false;
            }
        }
        ...
        if (0 != ifIndex)
        {
            ...
            {
                DMSG(0, "LinuxRouteMgr::DeleteRoute() error adding RTA_GATEWAY attr.\n");
                return false;
            }  
        }
        ...
        if ((int)req.msg.nlmsg_len != result)
        {
            DMSG(0, "LinuxRouteMgr::DeleteRoute() send() error: %s\n",
                    strerror(errno));
            return false;   
        }
        ...
        if (!NetlinkCheckResponse(seq))
        {
            DMSG(0, "LinuxRouteMgr::DeleteRoute() error deleting route\n");
            return false;   
        }
    }
    return true;
...
{   
    ...
    do
    {
        ...
        if (!buffer)
        {
            DMSG(0, "LinuxRouteMgr::GetRoute() new buffer error: %s\n", strerror(errno));
            return false;   
        }
...
#endif // HAVE_IPV6
            default:
                DMSG(0, "LinuxRouteMgr::GetRoute() invalid destination address!\n");
                delete[] buffer;
                return false;
                break;
        }
        ...
        if (dst.IsValid())
        {
            ...
            if (prefixLen < addrBits)
            {
                ...
                if (0 != ((0x00ff >> (prefixLen & 0x07)) & bytes[index]))
                {
                    DMSG(0, "LinuxRouteMgr::GetRoute() invalid address for given mask\n");
                    delete[] buffer;
                    return false;   
                }
                while (++index < dst.GetLength())
            ...
            {
                DMSG(0, "LinuxRouteMgr::GetRoute() invalid mask.\n");
                delete[] buffer;
                return false;
            }
            if (!NetlinkAddAttr(&req.msg, sizeof(req), RTA_DST, dst.GetRawHostAddress(), dst.GetLength()))
            {
                DMSG(0, "LinuxRouteMgr::GetRoute() error setting RTA_DST attr.\n");
                delete[] buffer;
                return false;
            }
        }
        ...
        {
            DMSG(0, "LinuxRouteMgr::GetRoute() no valid destination given\n");
            delete[] buffer;
            return false;
        }
        ...
        if (result < 0)
        {
            DMSG(0, "LinuxRouteMgr::GetRoute() send() error: %s\n",
                    strerror(errno));
            delete[] buffer;
            return false;   
        }
        ...
        while(!done)
        {
            ...
            if (msgLen < 0)
            {
                DMSG(0, "LinuxRouteMgr::GetRoute() recv() error: %s\n", strerror(errno)); 
                delete[] buffer;
                return false;  
            }
            ...
            for (; 0 != NLMSG_OK(msg, (unsigned int)msgLen); msg = NLMSG_NEXT(msg, msgLen))
            {
                ...
                {       
                    ...
                    switch (msg->nlmsg_type)
                    {
                        ...
                        {
                            struct nlmsgerr* errorMsg = (struct nlmsgerr*)NLMSG_DATA(msg);
                            DMSG(1, "LinuxRouteMgr::GetRoute() recvd matching NLMSG_ERROR error seq:%d code:%d...\n", 
                                    msg->nlmsg_seq, errorMsg->error);
                            delete[] buffer;
                            return false;
                            break;
                ...
                else
                {
                    ...
                    {
                        struct nlmsgerr* errorMsg = (struct nlmsgerr*)NLMSG_DATA(msg);
                        DMSG(1, "LinuxRouteMgr::GetRoute() recvd non-matching NLMSG_ERROR seq:%d code:%d\n", 
                                msg->nlmsg_seq, errorMsg->error);  
                    }
                                 ...
                                 ProtoRouteTable&   routeTable)
{
    ...
    do
    {
        ...
        if (!buffer)
        {
            DMSG(0, "LinuxRouteMgr::GetAllRoutes() new buffer error: %s\n", strerror(errno));
            return false;   
        }
...
#endif // HAVE_IPV6
            default:
                DMSG(0, "LinuxRouteMgr::GetAllRoutes() invalid address type\n");
                delete[] buffer;
                return false;
        }
        ...
        if (result < 0)
        {
            DMSG(0, "LinuxRouteMgr::GetAllRoutes() send() error: %s\n", strerror(errno));
            delete[] buffer;
            return false;   
        }
        ...
        while(!done)
        {
            ...
            if (msgLen < 0)
            {
                DMSG(0, "LinuxRouteMgr::GetAllRoutes() recv() error: %s\n", 
                                strerror(errno));
                delete[] buffer;
                return false;  
            } 
            ...
            { 
                ...
                {          
                    ...
                    if (0 != (NLM_F_MULTI & msg->nlmsg_flags))
                    {
                        //TRACE("multi part message ...\n");
                    }
                    switch (msg->nlmsg_type)
                    {
                        case NLMSG_NOOP:
                        ...
                        {
                            struct nlmsgerr* errorMsg = (struct nlmsgerr*)NLMSG_DATA(msg);
                            DMSG(0, "LinuxRouteMgr::GetAllRoutes() recvd NLMSG_ERROR error seq:%d code:%d...\n", 
                                    msg->nlmsg_seq, errorMsg->error);
                            break;
                        }
                        case NLMSG_DONE:
                        ...
                        case RTM_NEWROUTE:
                        {
...
#endif // HAVE_IPV6
                                default:
                                    DMSG(0, "LinuxRouteMgr::GetAllRoutes() recvd invalid addr family\n");
                                    delete[] buffer;
                                    return false;
                                    break;
                            }
                                ...
                                (0 == (RTM_F_CLONED & rtm->rtm_flags)))
                            {
                                ...
                                {
                                    DMSG(0, "LinuxRouteMgr::GetAllRoutes() error creating table entry\n");
                                    delete[] buffer;
                                    return false;   
                                }
                            }
                ...
                else
                {
                    ...
                    {
                        struct nlmsgerr* errorMsg = (struct nlmsgerr*)NLMSG_DATA(msg);
                        DMSG(0, "LinuxRouteMgr::GetAllRoutes() recvd NLMSG_ERROR seq:%d code:%d\n", 
                                msg->nlmsg_seq, errorMsg->error);  
                    }
                }
            }  // end for(NLMSG_NEXT())
...
bool LinuxRouteMgr::GetInterfaceAddressList(unsigned int        ifIndex, 
                                            ...
                                            ProtoAddress::List& addrList)
{
    ProtoAddress::List localAddrList;  // keep link/site local addresses separate and add to "addrList" at end
...
#endif // HAVE_IPV6
        default:
            DMSG(0, "LinuxRouteMgr::GetInterfaceAddressList() invalid destination address!\n");
            return false;
            break;
    }
    ...
    if (result < 0)
    {
        DMSG(0, "LinuxRouteMgr::GetInterfaceAddressList() send() error: %s\n",
                strerror(errno));
        return false;   
    }
    ...
    while(!done)
    {
        ...
        if (msgLen < 0)
        {
            DMSG(0, "LinuxRouteMgr::GetInterfaceAddressList() recv() error: %s\n", strerror(errno)); 
            return false;  
        }
        struct nlmsghdr* msg = (struct nlmsghdr*)buffer;
        for (; 0 != NLMSG_OK(msg, (unsigned int)msgLen); msg = NLMSG_NEXT(msg, msgLen))
        {
            ...
            {          
                switch (msg->nlmsg_type)
                {
                    ...
                    {
                        struct nlmsgerr* errorMsg = (struct nlmsgerr*)NLMSG_DATA(msg);
                        DMSG(0, "LinuxRouteMgr::GetInterfaceAddressList() recvd NLMSG_ERROR error seq:%d code:%d...\n", 
                                msg->nlmsg_seq, errorMsg->error);
                        return false;
                        break;
                    }
                                        ...
                                        switch (ifa->ifa_scope)
                                        {
                                            case RT_SCOPE_UNIVERSE: 
                                            {
                                                ...
                                                if (theAddress.IsValid())
                                                {
                                                    ...
                                                    {
                                                        DMSG(0, "LinuxRouteMgr::GetInterfaceAddressList() error: couldn't add to addrList\n");
                                                        done = true;
                                                    }
                                                }
                                                break;
                                            ...
                                            case RT_SCOPE_LINK:
                                            {
                                                ...
                                                if (theAddress.IsValid() && !localAddrList.Insert(theAddress))
                                                    DMSG(0, "LinuxRouteMgr::GetInterfaceAddressList() error: couldn't add to localAddrList\n");
                                                break;
                                            }
                                            default:
                    ...
                    default:
                        DMSG(0, "LinuxRouteMgr::GetInterfaceAddressList() matching reply type:%d len:%d bytes\n", 
                                msg->nlmsg_type, msg->nlmsg_len);
                        break;
                }  // end switch(nlmsg_type)
            }
            else
            {
                ...
                {
                    struct nlmsgerr* errorMsg = (struct nlmsgerr*)NLMSG_DATA(msg);
                    DMSG(0, "LinuxRouteMgr::GetInterfaceAddressList() recvd NLMSG_ERROR seq:%d code:%d\n", 
                            msg->nlmsg_seq, errorMsg->error);  
                }
            }  // end if/else (matching pid && seq)
        }  // end for(NLMSG_NEXT())
    ...
    while (iterator.GetNextAddress(localAddr))
    {
        ...
        {
            DMSG(0, "LinuxRouteMgr::GetInterfaceAddressList() error: couldn't add localAddr to addrList\n");
            break;
        }
    }    
    if (0 != addrList.GetCount())
        DMSG(4, "LinuxRouteMgr::GetInterfaceAddressList() warning: no addresses found\n");
    return true;
