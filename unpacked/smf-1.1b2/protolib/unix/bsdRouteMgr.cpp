...
bool BsdRouteMgr::Open(const void* /*userData*/)
{
    ...
    {
        DMSG(0, "BsdRouteMgr::Open() socket(AF_ROUTE) error: %s\n",
                strerror(errno));  
        return false; 
    }
    else
                               ...
                               ProtoRouteTable&   routeTable)
{
...
#endif // HAVE_IPV6
        default:
            DMSG(0, "BsdRouteMgr::GetAllRoutes() unsupported addr family\n");
            return false;
    }
    mib[4] = NET_RT_DUMP;
    mib[5] = 0;
    ...
    {
        DMSG(0, "BsdRouteMgr::GetAllRoutes() sysctl() error: %s\n",
                strerror(errno));
        return false;   
    }   
    ...
    if (!(buf = new char[len]))
    {
        DMSG(0, "BsdRouteMgr::GetAllRoutes() malloc(%d) error: %s\n",
                len, strerror(errno));
        return false;
    }
    ...
    while (next < end)
    {
        ...
        for (int i = 0; i < RTAX_MAX; i++)
        {
            if (0 != (rtm->rtm_addrs & (0x01 << i)))
            {
                switch (i)
                {
                    ...
                    case RTAX_GENMASK:
                    {
                        DMSG(0, "BsdRouteMgr::GetAllRoutes() recvd RTA_GENMASK ...\n");
                        break;
                    }
                    default:
                    {
                        DMSG(0, "BsdRouteMgr::GetAllRoutes() recvd unhandled RTA: %d\n", i);
                        break;   
                    }
                }  // end switch(i)
                NEXT_SA(addr);
            ...
            (0 == (rtm->rtm_flags & RTF_WASCLONED)))
        {
            ...
            {
                DMSG(0, "BsdRouteMgr::GetAllRoutes() error creating table entry\n");
                delete[] buf;
                return false;   
            }
        }
                           ...
                           int                 /*metric*/)
{
...
#endif // HAVE_IPV6
        default:
            DMSG(0, "BsdRouteMgr::SetRoute() invalid dst address type!\n");
            return false; 
    }
    ...
    if (prefixLen < (unsigned int)(dst.GetLength() << 3))
    {
        ...
        {
            DMSG(0, "BsdRouteMgr::SetRoute() invalid address for given mask\n");
            return false;   
        }
    ...
    {
        DMSG(0, "BsdRouteMgr::SetRoute() invalid gateway address\n");
        return false;   
    }
    ...
    {
        DMSG(0, "BsdRouteMgr::SetRoute() send() error: %s\n",
                strerror(errno));
        return false;   
    } 
    ...
    while (1)
    {
        ...
        if (msgLen < 0)
        {
            if (errno != EINTR)
            {
                DMSG(0, "BsdRouteMgr::SetRoute() recv() error: %s\n", strerror(errno)); 
                return false;  
            }
            else
            {
            ...
            (pid == rtm->rtm_pid))
        {
            ...
            if (0 != (rtm->rtm_flags & RTF_DONE))
            {
                ...
                {
                    DMSG(0, "BsdRouteMgr::SetRoute() completed with invalid dst\n");
                    return false;
                } 
            }
        }
                              ...
                              unsigned int        ifIndex)
{
...
#endif // HAVE_IPV6
        default:
            DMSG(0, "BsdRouteMgr::DeleteRoute() invalid dst address type!\n");
            return false; 
    }
    struct rt_msghdr* rtm = (struct rt_msghdr*)buffer;
    rtm->rtm_msglen = sizeof(struct rt_msghdr);
    ...
    if (prefixLen < (unsigned int)(dst.GetLength() << 3))
    {
        ...
        if (0 != ((0x00ff >> (prefixLen & 0x07)) & ptr[index]))
        {
            DMSG(0, "BsdRouteMgr::DeleteRoute() invalid address for given mask\n");
            return false;   
        }
        while (++index < dst.GetLength())
        {
    ...
    if ((int)rtm->rtm_msglen != result)
    {
        ...
        DMSG(4, "BsdRouteMgr::DeleteRoute() send() warning: %s\n",
                strerror(errno));
        return false;   
    } 
    ...
    while (!complete)
    {
        ...
        if (msgLen < 0)
        {
            if (errno != EINTR)
            {
                DMSG(0, "BsdRouteMgr::DeleteRoute() recv() error: %s\n", strerror(errno)); 
                return false;  
            }
            else
            {
            ...
            (pid == rtm->rtm_pid))
        {
            ...
            if (0 != (rtm->rtm_flags & RTF_DONE))
            {
                ...
                {
                    DMSG(0, "BsdRouteMgr::DeleteRoute() completed with invalid dst\n");
                    return false;
                } 
            }
        }
                           ...
                           int&                metric)
{
...
#endif // HAVE_IPV6
        default:
            DMSG(0, "BsdRouteMgr::GetRoute() invalid dst address type!\n");
            return false; 
    }
    struct rt_msghdr* rtm = (struct rt_msghdr*)buffer;
    rtm->rtm_msglen = sizeof(struct rt_msghdr);
    ...
    if (prefixLen < (unsigned int)(dst.GetLength() << 3))
    {
        ...
        if (0 != ((0x00ff >> (prefixLen & 0x07)) & ptr[index]))
        {
            DMSG(0, "BsdRouteMgr::GetRoute() invalid address for given mask\n");
            return false;   
        }
        while (++index < dst.GetLength())
        {
    ...
    if ((int)rtm->rtm_msglen != result)
    {
        DMSG(0, "BsdRouteMgr::GetRoute() send() error: %s\n", strerror(errno));
        return false;   
    }  
    ...
    while (1)
    {
        ...
        if (msgLen < 0)
        {
            if (errno != EINTR)
            {
                DMSG(0, "BsdRouteMgr::GetRoute() recv() error: %s\n", strerror(errno)); 
                return false;  
            }
            else
            {
                        ...
                        case RTAX_GATEWAY:
                        {
                            ...
                            {   
                                ...
                                case AF_LINK:
                                    ...
                                    break;
                                default:
                                    DMSG(0, "BsdRouteMgr::GetRoute() recvd unknown sa_family\n");
                                    break;
                            }
                            break;
                        }
...
bool BsdRouteMgr::GetInterfaceAddressList(unsigned int        ifIndex, 
                                          ...
                                          ProtoAddress::List& addrList)
{
    ProtoAddress::List localAddrList;  // keep link & site local addrs separate and add at end
    // Init return values
    int mib[6];
    mib[0] = CTL_NET;
    mib[1] = AF_ROUTE;
...
#endif // HAVE_IPV6
        default:
            DMSG(0, "BsdRouteMgr::GetInterfaceAddressList() unsupported addr family\n");
            break;
    }
    mib[4] = NET_RT_IFLIST;
    mib[5] = ifIndex;
    ...
    if (sysctl(mib, 6, NULL, &len, NULL, 0) < 0)
    {
        DMSG(0, "BsdRouteMgr::GetInterfaceAddressList() sysctl() error: %s\n",
                strerror(errno));
        return false;   
    }   
    ...
    if (!(buf = new char[len]))
    {
        DMSG(0, "BsdRouteMgr::GetInterfaceAddressList() malloc(%d) error: %s\n",
                len, strerror(errno));
        return false;
    }
    ...
    while (next < end)
    {
        ...
        switch (ifm->ifm_type)
        {
                                    ...
                                    if (addrTemp.IsValid())
                                    {
                                        if (addrTemp.IsLinkLocal() || addrTemp.IsSiteLocal())
                                        {
                                            if (!localAddrList.Insert(addrTemp))
                                                DMSG(0, "BsdRouteMgr::GetInterfaceAddressList() error: unable to add addr to local list\n");
                                        }
                                        else
                                        {
                                            if (!addrList.Insert(addrTemp))
                                                DMSG(0, "BsdRouteMgr::GetInterfaceAddressList() error: unable to add addr to list\n");
                                        }   
                                    }
                                }
                                break;
            ...
            default:
                DMSG(0, "BsdRouteMgr::GetInterfaceAddressList() warning: unhandled IFM message type:%d\n", ifm->ifm_type);
                break;
    ...
    while (iterator.GetNextAddress(localAddr))
    {
        if (!addrList.Insert(localAddr))
            DMSG(0, "BsdRouteMgr::GetInterfaceAddressList() error: unable to add local addr to list\n");
    }         
    if (0 != addrList.GetCount())
        DMSG(4, "BsdRouteMgr::GetInterfaceAddressList() warning: no addresses found\n");
    return true;
