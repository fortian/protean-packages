...
class ProtoRouteTable
{
    public:
        ...
        {
            Destroy();
            // min keysize equals min mask size (1 bit)
            // max keysize equals IPv6 address size (128 bits)
            return tree.Init((unsigned int)1, (unsigned int)128);
        }
        bool IsReady() const {return tree.IsReady();}
        void Clear();
        void Destroy();   
        ...
        class Entry : public ProtoTree::Item
        {
            ...
            public:
                ...
                unsigned int GetPrefixLength() const {return keysize;}  // in bits
            ...
            private:
                Entry();
                Entry(const ProtoAddress& dstAddr, unsigned int prefixLen);
                void Init(const ProtoAddress& dstAddr, unsigned int prefixLen);
