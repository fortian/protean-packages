...
// This example performs asynchronous I/O on a libpcap device
...
bool PcapExample::OnStartup(int argc, const char*const* argv)
{
    ...
    {
        DMSG(0, "pcapExample: Error! bad command line\n");
        return false;
    }
    return true;
...
bool PcapExample::ProcessCommands(int argc, const char*const* argv)
{
    ...
    while ( i < argc)
    {
        ...
        switch (GetCmdType(argv[i]))
        {
            case CMD_INVALID:
            {
                DMSG(0, "PcapExample::ProcessCommands() Invalid command:%s\n", 
                        argv[i]);
                return false;
            }
            case CMD_NOARG:
                ...
                {
                    DMSG(0, "PcapExample::ProcessCommands() ProcessCommand(%s) error\n", 
                            argv[i]);
                    return false;
                }
                i++;
            ...
            case CMD_ARG:
                ...
                {
                    DMSG(0, "PcapExample::ProcessCommands() ProcessCommand(%s, %s) error\n", 
                            argv[i], argv[i+1]);
                    return false;
                }
                i += 2;
...
bool PcapExample::OnCommand(const char* cmd, const char* val)
{
    ...
    if ((CMD_ARG == type) && !val)
    {
        DMSG(0, "PcapExample::ProcessCommand(%s) missing argument\n", cmd);
        return false;
    }
    else if (!strncmp("listen", cmd, len))
    {
        ...
        {
            DMSG(0, "pcapExample: invalid <interfaceName>\n");
            return false;
        }
        ...
        if (0 != strlen(errbuf))
            DMSG(0, "pcapExample: pcap_open_live() warning: %s\n", errbuf);
        ...
        if (!dispatcher.InstallGenericInput(pcapDescriptor,
                                            ...
                                            this))
        {
            DMSG(0, "pcapExample: error installing pcap input\n");
            pcap_close(pcap_device);
            pcap_device = NULL;
            return false;
        }
    ...
    else if (!strncmp("forward", cmd, len))
    {
        ...
        {
            DMSG(0, "pcapExample: error creating packeteer: %s\n", GetErrorString());
            return false;    
        }    
        if (!packeteer->Open(val))
        {
            DMSG(0, "pcapExample: error opening packeteer!\n");   
            return false;
        }   
    }
    return true;
...
void PcapExample::OnPcapInputReady()
{
    ...
    switch (pcap_next_ex(pcap_device, &hdr, &data))
    {
        ...
        default:    // error (or EOF for offline)
            DMSG(0, "PcapExample::DoPcapInput() pcap_next_ex() error\n");
            break;
    } 
