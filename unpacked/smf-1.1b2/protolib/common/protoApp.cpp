...
bool ProtoApp::ProcessCommandString(const char* cmdline)
{
    ...
    if (argv) 
    {
        ...
        {
	        delete[] argv;
	        DMSG(0, "protoApp: memory allocation error: %s\n", GetErrorString());
	        return false;
        }
    ...
    while (*ptr != '\0')
    {
        ...
        if (1 == sscanf(ptr, "%s", fieldBuffer))
        {
            ...
            if ('"' == fieldBuffer[0])
            {
                ...
                if (!end)
                {
                    DMSG(0, "protoApp: Error parsing command line: Unterminated quoted string\n");
...
{
    // This WIN32 code lets us build a console app if we like
