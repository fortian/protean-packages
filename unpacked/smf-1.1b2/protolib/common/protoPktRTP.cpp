                                 ...
                                 bool           freeOnDestruct)
{
    if (ProtoPkt::InitFromBuffer(pktLength, bufferPtr, bufferBytes, freeOnDestruct))  // sets "pkt_length"
    {
        ...
        {
            if (NULL != buffer_ptr)
                DMSG(0, "ProtoPktRTP::InitFromBuffer() error: insufficient buffer_ptr space (1)\n");
            return false;
        }
        if (VERSION != GetVersion())
        {
            DMSG(0, "ProtoPktRTP::InitFromBuffer() error: incompatible version number: %d\n", GetVersion());
            return false;
        }
        unsigned int hdrLength = BASE_HDR_LEN + GetCsrcCount()*sizeof(UINT32) + GetExtensionLength();
        if (pktLength < hdrLength)
        {
            DMSG(0, "ProtoPktRTP::InitFromBuffer() error: bad RTP header for given pkt_length\n");
        }
    ...
    {
        DMSG(0, "ProtoPktRTP::InitFromBuffer() error: insufficient buffer_ptr space (2)\n");
	    return false;
    }
...
bool ProtoPktRTP::Init(UINT32* bufferPtr, unsigned int bufferBytes, bool freeOnDestruct)
{
   ...
   {
       if (NULL != bufferPtr)
           DMSG(0, "ProtoPktRTP::Init() error: insufficient buffer_ptr space\n");
       pkt_length = 0;
       return false;
   }
...
bool ProtoPktRTP::AppendCsrc(UINT32 csrcId)
{
	...
	{
        DMSG(0, "ProtoPktRTP::AppendCsrc() error: insufficient buffer_ptr or max CSRC count exceeded\n");
...
bool ProtoPktRTP::PackExtension(Extension& extension)
{
    ...
    {
        DMSG(0, "ProtoPktRTP::PackExtension() error: insufficient buffer_ptr space\n");
        return false;
    }
}
                                            ...
                                            bool            freeOnDestruct)
{
    ...
    if (NULL != buffer_ptr)
        DMSG(0, "ProtoPktRTP::Extension::InitFromBuffer() error: insufficient buffer space\n");
    return false;
}  // end ProtoPktRTP::InitFromBuffer()
                                  ...
                                  bool          freeOnDestruct)
{
    ...
    {
        DMSG(0, "ProtoPktRTP::Extension::Init() error: insufficient buffer space\n");
        return false;
    }
...
bool ProtoPktRTP::Extension::SetData(const char* dataPtr, unsigned int numBytes)
{
    ...
    {
        DMSG(0, "ProtoPktRTP::Extension::SetData() error: insufficient buffer space\n");
        return false;
    }
