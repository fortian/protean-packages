...
// These classes provide representations of IP packet formats
...
class ProtoPktIPv4 : public ProtoPktIP
{
    public:
        ...
        enum Flag
        {
            FLAG_RESERVED = 0x40,  // reserved bit
            FLAG_DF       = 0x20,  // 0 = may fragment,  1 = don't fragment 
            FLAG_MF       = 0x10   // 0 = last fragment, 1 = more fragments
        };
        ...
        UINT16 GetFragmentOffset() const
            {return (0x1fff & ((UINT16*)buffer_ptr)[OFFSET_FRAGMENT]);}
        UINT8 GetTTL() const
        ...
        // Helper methods for UDP checksum calculation
        const UINT32* GetAddrPtr() const {return (buffer_ptr + OFFSET_SRC_ADDR);}
        ...
        // updates IPv4 header checksum when a byte
        // is changed.
    ...
    public:
        ...
        class Option
        {
            public:
                ...
                const char* GetBuffer() const {return buffer_ptr;}
                char* AccessBuffer() const {return buffer_ptr;}
        ...
        class Extension : public ProtoPkt
        {
            public:
                ...
                Option* AddOption(Option::Type optType);
...
class ProtoPktAH : public ProtoPkt
{
    
};  // end class ProtoPktAH

// IPv6 Authentication Header (AUTH) extension
...
// IPv6 Encapsulating Security Protocol (ESP) header
...
// IPv6 Simplified Multicast Forwarding Duplicate Packet Detection (DPD) option
class ProtoPktDPD : public ProtoPktIPv6::Option
{
    public:
        ...
        TaggerIdType GetTaggerIdType() const
            {return ((TaggerIdType)((buffer_ptr[OFFSET_TID_TYPE] >> 4) & 0x0f));}
        
        UINT8 GetTaggerIdLength() const
        {
            UINT8 tidType = buffer_ptr[OFFSET_TID_TYPE];
            return ((0 != tidType) ? ((tidType & 0x0f) + 1) : 0);  
        }
        ...
        UINT8 GetPktIdLength() const
            {return (GetDataLength() - GetTaggerIdLength() - 1);}
    ...
    private:
        ...
        unsigned int OffsetPktId() const
            {return (OFFSET_TID_VALUE + GetTaggerIdLength());}
...
class ProtoPktUDP : public ProtoPkt
{
    public:
        ...
        const UINT32* GetPayload()
            {return (buffer_ptr + OFFSET_PAYLOAD/4);}
        ...
        UINT16 CalculateChecksum(ProtoPktIP& ipHdr, bool set = true);
        
    private:
        enum
        {
            OFFSET_SRC      = 0,                 // source port number
            OFFSET_DST      = OFFSET_SRC + 1,    // destination port number
            OFFSET_LENGTH   = OFFSET_DST + 1,    // UDP datagram length (bytes)
            OFFSET_CHECKSUM = OFFSET_LENGTH + 1, // 
            OFFSET_PAYLOAD  = (OFFSET_CHECKSUM + 1)/2
        };
