...
//  This example illustrates the use of threaded ProtoDispatcher operation
...
bool Ticker::Start()
{
    ...
    {
        DMSG(0, "threadExample: Ticker::Start() thread start failure ...\n");
        timer.Deactivate();
        return false;
    }      
}  // end  Ticker::Start()
...
{
    // IMPORTANT NOTE: Remember this timeout handler
    ...
    DMSG(0, "threadExample: Ticker::OnTimeout() ...\n");
    return true;
...
int main(int argc, char* argv[])
{
    ...
    if (!ticker.Start())
    {
        DMSG(0, "threadExample: Ticker::Start() error\n");
        return -1;   
    }
    ...
    while (count--)
    {
        ...
        if (count)
        {
            DMSG(0, "threadExample: 3 seconds have passed (starting ticker timer)\n");        
            ticker.StartTimer();
        }
        else
        {   
            DMSG(0, "threadExample: 3 more seconds have passed (stopping ticker timer)\n");        
            ticker.Stop();
        }
        
    }
    DMSG(0, "threadExample: ticker stopped.\n");
