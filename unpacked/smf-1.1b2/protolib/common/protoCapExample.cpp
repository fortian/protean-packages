...
// This example uses a ProtoCap class instance to sniff for
...
bool ProtoCapExample::OnStartup(int argc, const char*const* argv)
{
    ...
    {
        DMSG(0, "protoCapExample::OnStartup() new ProtoCap cap_rcvr error: %s\n", GetErrorString());
        return false;
    }  
    ...
    {
        DMSG(0, "protoCapExample::OnStartup() new ProtoCap cap_sndr error: %s\n", GetErrorString());
        return false;
    }  
    
    if (!ProcessCommands(argc, argv))
    {
        DMSG(0, "protoCapExample: Error! bad command line\n");
        return false;
    }
    ...
    if (sched_setscheduler(0, SCHED_FIFO, &schp))
    {
        ...
        if (sched_setscheduler(0, SCHED_OTHER, &schp))
            DMSG(0, "protoCapExample: Warning! Couldn't set any real-time priority: %s\n", GetErrorString());
    }
#endif // HAVE_SCHED
    ...
    if (!SetPriorityClass(GetCurrentProcess(), REALTIME_PRIORITY_CLASS))
	    DMSG(0, "protoCapExample: Warning! SetPriorityClass() error: %s\n", GetErrorString());
#endif // !_WIN32_WCE
	if (!SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_TIME_CRITICAL))
	    DMSG(0, "protoCapExample: Warning! SetThreadPriority() error: %s\n", GetErrorString());
#endif // WIN32
...
bool ProtoCapExample::ProcessCommands(int argc, const char*const* argv)
{
    ...
    while ( i < argc)
    {
        ...
        switch (GetCmdType(argv[i]))
        {
            case CMD_INVALID:
            {
                DMSG(0, "ProtoCapExample::ProcessCommands() Invalid command:%s\n", 
                        argv[i]);
                return false;
            }
            case CMD_NOARG:
                ...
                {
                    DMSG(0, "ProtoCapExample::ProcessCommands() ProcessCommand(%s) error\n", 
                            argv[i]);
                    return false;
                }
                i++;
            ...
            case CMD_ARG:
                ...
                {
                    DMSG(0, "ProtoCapExample::ProcessCommands() ProcessCommand(%s, %s) error\n", 
                            argv[i], argv[i+1]);
                    return false;
                }
                i += 2;
...
bool ProtoCapExample::OnCommand(const char* cmd, const char* val)
{
    ...
    if ((CMD_ARG == type) && !val)
    {
        DMSG(0, "ProtoCapExample::ProcessCommand(%s) missing argument\n", cmd);
        return false;
    }
    else if (!strncmp("listen", cmd, len))
    {
        ...
        {
            DMSG(0, "protoCapExample: invalid <interfaceName>\n");
            return false;
        }
        if (!cap_rcvr->Open(ifName))
        {
            DMSG(0, "protoCapExample: ProtoCap::Open() error.\n");
            return false;   
        }
    ...
    else if (!strncmp("forward", cmd, len))
    {
        if (!cap_rcvr->IsOpen())
        {
            DMSG(0, "protoCapExample: command line error: must specify \"listen\" first\n");
            Usage();
            return false;
        }
        // Make sure "val" is an interface name
    ...
    {
        DMSG(0, "protoCapExample:: invalid command\n");
        return false;
    }
    return true;
                                ...
                                ProtoChannel::Notification notifyType)
{
    if (ProtoChannel::NOTIFY_INPUT == notifyType)
    {
        ...
        do
        {
            ...
            {
                DMSG(0, "protoCapExample::OnReceive() ProtoCap::Recv() error\n");
                break;
            } 
            else if (0 != numBytes)
            {
