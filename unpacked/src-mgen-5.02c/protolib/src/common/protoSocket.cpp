...
bool ProtoSocket::HostIsIPv6Capable()
{
    ...
    {
        // search for an ipv6 address assignment
        struct ifaddrs *ifa, *first;
        bool has_ipv6 = false;
        if (getifaddrs(&first) < 0) 
        {
            PLOG(PL_ERROR,"ProtoSocket::HostIsIPv6Capable(): error in getifaddres\n");
            return false;
        }
        for (ifa = first; ifa; ifa = ifa->ifa_next)
        {
            has_ipv6 |= (ifa->ifa_addr->sa_family == AF_INET6);
            if (has_ipv6)
                break;
        }
        freeifaddrs(first);
