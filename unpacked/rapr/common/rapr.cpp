...
bool Rapr::Start()
{
    if (!GetNumberGenerator().GetInitialSeed() && !hostIDSet)
      DMSG(0,"Rapr::Start() Warning: Default HostID %hi was not overridden and has been used as the initial seed.\n",GetNumberGenerator().GetHostID());
    ...
    if (start_sec < 0.0)
    {
        ...
        if (log_file)
        {
            ...
            struct tm* timePtr = localtime((time_t*)&currentTime.tv_sec);
    ...
    case INITIAL_SEED:
      {
          ...
          if (hostIDSet)
          {
              DMSG(0,"Rapr::OnCommand() Error: INITIAL_SEED must be specified BEFORE HOSTID\n");
              return false;
          }
...
bool Rapr::ParseBehaviorEvent(const char* lineBuffer, unsigned int lineCount,RaprDictionaryTransfer* trans,BehaviorEvent::EventSource eventSource,RaprPRNG* prng,char* origLineBuffer)
{
...
bool Rapr::ParseEvent(const char* lineBuffer, unsigned int lineCount,RaprDictionaryTransfer* trans,BehaviorEvent::EventSource eventSource,RaprPRNG* prng,char* origLineBuffer)
{
    const char * ptr = lineBuffer;
    if (!ptr) return true;
