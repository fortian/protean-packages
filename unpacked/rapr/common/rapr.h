...
class Rapr
{
 public:
  ...
  enum Command
    {
      ...
      LOCALTIME, // turn on localtime logging in mgen
      NOLOG,     // no output
  ...
  bool ParseBehaviorEvent(const char* lineBuffer, unsigned int lineCount,RaprDictionaryTransfer* trans,BehaviorEvent::EventSource eventSource,RaprPRNG* prng,char* origLineBuffer = NULL);
  bool ParseEvent(const char* lineBuffer, unsigned int lineCount,RaprDictionaryTransfer* trans,BehaviorEvent::EventSource eventSource,RaprPRNG* prng,char* origLineBuffer = NULL);
