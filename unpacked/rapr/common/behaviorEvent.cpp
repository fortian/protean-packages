...
BehaviorEvent::BehaviorEvent(ProtoTimerMgr& timerMgr,
  ...
  : rapr(theRapr),inUse(true),type(INVALID_TYPE),
    ...
    option_mask(0),logic_id_mask(0),raprFlowID(0)
{
}
...
char* BehaviorEvent::LookupDictionaryEntry(char* buf)
{
...
char* BehaviorEvent::ParsePattern(char* ptr,char* fieldBuffer)
{
    ...
    // rebuild "our" patternString
    sprintf(patternString,fieldBuffer);
...
bool BehaviorEvent::ParseOptions(const char* string)
{
    ...
    while ('\0' != *ptr)
    {
        ...
        case PATTERN:
          {
              char *tmpPtr = const_cast<char*>(ptr);
...
bool RaprEvent::OnStartUp()
{
    ...
    case FAULT:
      {
          ...
          {
          case STOP_RAPR:
            sprintf(msgBuffer,"type>application action>fault type>STOP_RAPR.");
            rapr.Stop();
            break;
          case STOP_MGEN:
            sprintf(msgBuffer,"type>application action>fault type>STOP_MGEN.");	
            rapr.StopMgen();
            break;
          case STOP_EVENTS:
            sprintf(msgBuffer,"type>application action>fault type>STOP_EVENTS.");
            rapr.StopEvents();
            break;
          case INVALID_FAULT:
            sprintf(msgBuffer,"type>application action>fault type>INVALID_FAULT.");
    ...
    case CLEAR:
      {
          ...
          sprintf(msgBuffer,"type>RaprEvent action>clearing objectType>%s ubi>%lu",GetStringFromEventType(clearEventType),ubi);
          rapr.LogEvent("RAPR",msgBuffer);
    ...
    case RAPRFLOWID:
      {
          ...
          if (theEvent)
          {
              sprintf(msgBuffer,"type>RaprEvent action>stopping RaprFlowID>%d",GetRaprFlowID());
              rapr.LogEvent("RAPR",msgBuffer);
    ...
    case DICTIONARY_ENTRY:
      {
          sprintf(msgBuffer,"type>RaprEvent action>resetting_dictionary_entry name>%s value>%s",name,value);
          rapr.LogEvent("RAPR",msgBuffer);
    ...
    case LOGICTABLE_FILE:
      {
          sprintf(msgBuffer,"type>RaprEvent action>loading_logictable name>%s",name);
          rapr.LogEvent("RAPR",msgBuffer);
          
          if (!rapr.LogicTable().LoadFile(name))
          {
              sprintf(msgBuffer,"type>Error action>loading_logictable name>%s",name);
              rapr.LogEvent("RAPR",msgBuffer);
              return false;
          }
          break;
    ...
    case DICTIONARY_FILE:
      {
          sprintf(msgBuffer,"type>RaprEvent action>loading_dictionary name>%s",name);
          rapr.LogEvent("RAPR",msgBuffer);
          
          if (!rapr.LogicTable().LoadDictionary(name))
          {
              sprintf(msgBuffer,"type>Error action>loading_dictionary name>%s",name);
              rapr.LogEvent("RAPR",msgBuffer);
              return false;
          }
    ...
    case CHANGE_STATE:
      {
          sprintf(msgBuffer,"type>RaprEvent action>change_state value>%s",name);
          rapr.LogEvent("RAPR",msgBuffer);
          if (!rapr.LogicTable().SetStateFromString(name))
          {
              sprintf(msgBuffer,"type>Error action>change_state name>%s",name);
              rapr.LogEvent("RAPR",msgBuffer);
              return false;
          }
          break;
    ...
    case CHANGE_UBI_STATE:
      {
          sprintf(msgBuffer,"type>RaprEvent action>change_ubi_state ubi>%s value>%s",name,value);
          rapr.LogEvent("RAPR",msgBuffer);
          if (!rapr.LogicTable().SetUBIState(name,value))
          {
              sprintf(msgBuffer,"type>Error action>change_ubi_state ubi>%s state>%s",name,value);
              rapr.LogEvent("RAPR",msgBuffer);
              return false;
          }
          break;
    ...
    case LOGICID:
      {
          sprintf(msgBuffer,"type>RaprEvent action>logic_id value>%s",name);
          rapr.LogEvent("RAPR",msgBuffer);
          int id = 0;
          id = atoi(name);
          if (id == 0) {
              sprintf(msgBuffer,"type>Error action>logic_id incorrect value>%s",name);
              rapr.LogEvent("RAPR",msgBuffer);
              return false;
              break;
          }
          ...
          {
              sprintf(msgBuffer,"type>Error action>logic_id value>%d",id);
              rapr.LogEvent("RAPR",msgBuffer);
              return false;
          }
          break;
    ...
    case RaprEvent::INPUT:
      {
          sprintf(msgBuffer,"type>RaprEvent action>loading_input_script value>%s",name);
          rapr.LogEvent("RAPR",msgBuffer);
          
          if (!rapr.ParseScript(name))
          {
              sprintf(msgBuffer,"type>Error action>loading_input_script value>%s",name);
              rapr.LogEvent("RAPR",msgBuffer);
              return false;
          }
...
bool Declarative::Stop()
{
    ...
    if (GetFlowID())
    {
        ...
        // log event
        char msgBuffer[512];
        sprintf(msgBuffer,"type>Declarative action>stopped ubi>%lu raprFlowId>%d ",ubi,raprFlowID);
...
bool Declarative::OnBehaviorTimeout(ProtoTimer& /*theTimer*/)
{
    ...
    // log event
    char msgBuffer[512];
    sprintf(msgBuffer,"type>Declarative action>timeout ubi>%lu ",ubi);
    strcat (msgBuffer,buffer);
    rapr.LogEvent("RAPR",msgBuffer);
...
bool Interrogative::OnStartUp()
{
    ...
    char msgBuffer[8192];
    sprintf(msgBuffer,"type>Interrogative action>start ubi>%lu eventSource>%s cnt>%d",ubi,GetStringFromEventSource(eventSource),retriesSent);
    ...
    if (rapr.GetVerbosity()) 
    {
        ...
        if (retriesSent)
        {
            sprintf(tmpBuffer," retriesSent>%d",retriesSent);
            strcat(msgBuffer,tmpBuffer);
        }
        if (GetSeed())
        {
    ...
    sprintf(tmpBuffer,"type>Interrogative action>off_event ubi>%lu eventSource>%s cnt>%d mgenCmd>\"%s\"",ubi,GetStringFromEventSource(eventSource),retriesSent,buffer);
    strcat(msgBuffer,tmpBuffer);
    rapr.LogEvent("RAPR",msgBuffer);
...
bool Interrogative::OnBehaviorTimeout(ProtoTimer& /*theTimer*/)
{
    ...
    flowID = 0; // Force onstartup to get a new flow id
...
bool Stream::OnStartUp()
{
    ...
    if (rapr.GetActiveStream() && rapr.GetActiveStream() != this)
    {
        ...
        {
            DMSG(0,"Stream::OnStartUp Warning: Another active stream>%lu has an equal or higher priority>%d myPriority>%d!  Stream> %lu not starting\n",rapr.GetActiveStream()->GetStreamUbi(),rapr.GetActiveStream()->GetBurstPriority(),burstPriority,streamUbi);
...
bool Stream::ProcessEvent(const MgenMsg& theMsg)
{
  ...
  if ((theMsg.GetPayload()) && (raprPayload.GetStreamSeq() < streamSeqNum))
    {
      DMSG(0,"Stream::ProcessEvent() Error: Out of order burst sequence number received>%d latest>%d\n",raprPayload.GetStreamSeq(),streamSeqNum);
      return false;
    }
  ...
  if (!burstCount)
    {
      DMSG(0,"Stream::Processevent() Burst count exceeded. ubi>%lu streamUbi>%lu streamSeqNum>%lu burstsSent>%lu burstCount>%lu\n",ubi,streamUbi,streamSeqNum,burstsSent,burstCount);
      return false;
    }
...
bool Stream::OnBurstTimeout(ProtoTimer& /*theTimer*/)
{
  ...
  for (unsigned int i = 0; i < streamFlowIdList.size(); i++)
    {
      ...
      // log event
      char msgBuffer[512];
      sprintf(msgBuffer,"type>Stream action>stopped_burst ubi>%lu streamUbi>%lu burstsSent>%d ",ubi,streamUbi,burstsSent);
      strcat (msgBuffer,buffer);
      rapr.LogEvent("RAPR",msgBuffer);
  ...
  if (rapr.GetActiveStream() == this)
    {
      DMSG(0,"Stream::OnBurstTimeout() Resetting active stream.\n");
      rapr.SetActiveStream(NULL);
    }
...
{    
    ...
    char* retranslatedCommand = strstr((*vec)[0],"INTERVAL");
    int cnt = 0;
