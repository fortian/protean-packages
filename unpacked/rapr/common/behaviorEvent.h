...
public:
  ...
  const char* GetStringFromEventSource(EventSource eventSource);
  const char* GetStringFromEventType(Type eventType);
  ...
  char* ParsePattern(char* ptr,char* fieldBuffer);
  ...
  char* LookupDictionaryEntry(char* buf);
...
class Periodic : public BehaviorEvent
{
 public:
  ...
  void SetOriginalCommand(char* ptr) {strcpy(origCommand,ptr);};
