...
RaprNumberGenerator::RaprNumberGenerator() 
{
  ...
  {
      char *ptr = strrchr(nodeAddress.GetHostString(),'.');
      hostid = atoi(++ptr);
      SetHostID(hostid);
  } else
  {
...
void RaprNumberGenerator::SetHostID(short host) {
    ...
    // If initial_seed is set, we have already
    // initialized the seeds - don't reinitialize.
    // Otherwise use the new hostID to do so.
    // (We ensure that initial_seed is the first cmd).
    if (!initialSeed)
    {
        InitializeSeeds(GetHostID());
    }
...
double RaprPRNG::GetRandom() {
  ...
  draw++;
  DMSG(1,"RaprPRNG::GetRandom() Draw: %d Seed: %d\n",draw,seed);
...
RaprPRNG::RaprPRNG() {
        DMSG(0,"RaprPRNG::RaprPRNG Error: No stream initialized!\n");
	seed = 0;  
	draw = 0;
	stream = NULL;
}
